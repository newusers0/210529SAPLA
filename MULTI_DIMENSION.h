...
#include "RTree_partition.h" //210618

TEMPLATE
class MULTI_DIMENSION : virtual public RTREE_PARTITION, public RTREE, virtual public TOOL, public APCA_QUAL, virtual public APCA_KNN_QUAL, public PLA_QUAL, public CHEBYSHEV_QUAL, public APLA, public APLA_ICDE07<DataType> {
public:
	struct MULTI_DIM;
	...
	MULTI_DIM multi_dim;
	//91201 for mixed dataset.
	struct TOOL::DATA_SOURCE data_source;
	struct TOOL::INPUT_ARGUMENT input_argument;
	struct TOOL::OUTPUT_ARGUMENT output_argument;//191206 sum deviation
	//200902 sum deviation, approximation time
	struct TOOL::RESULT_RECORD result_record;

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Tree ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	RTREE RTree;
	/*----------------210618--------------------------------*/
	//RTree_partition<DataType, ElementType> RTree_partition_object;
	RTREE_PARTITION RTree_partition_object;
	/*------------------------------------------------------*/
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

	/*---------------------------------------Old Version-------------------------------------------------*/
	vector<RTREE::Rect> rtree_rectangle_vector;// for insert Rtree.
	struct APCA_QUAL::APCA* apca_MBR = nullptr;// Multi::apca_MBR just use to insert Rtree, can be deteletd
	/*---------------------------------------------------------------------------------------------------*/
	//struct APCA_QUAL::APCA apca;
	//struct PLA_QUAL::PLA pla;
	//struct CHEBYSHEV_QUAL::CHEBYSHEV chebyshev;
	struct CHEBYSHEV_QUAL::CHEBYSHEV_SHARE chebyshev_share;
	struct APCA_QUAL::APCA_ARRAY* apca_array = nullptr;// for leaf node distance LB
	struct APCA_QUAL::APCA* apca_point_array = nullptr;// for leaf node distance LB
	struct PLA_QUAL::PLA* pla_array = nullptr;//for leaf node distance LB
	struct CHEBYSHEV_QUAL::CHEBYSHEV* chebyshev_array = nullptr;//for leaf node distance LB

	//191111 APLA vector
	std::vector<DoublyLinkedList<APLA::AREA_COEFFICIENT_SPEED_NO_MINMAX>> apla_array_vector;//for leaf node distance LB
	...
	*/
	...
	// For initialize of MULTI Class, apply memory for Class parameter APCA Array, PLA Array & Chebyshev Array, APLA& ICDE07 vector arry.
	void initialMultiDimension();
	// void initialMultiDimension(const typename TOOL::INPUT_ARGUMENT& input_argument, MULTI::MULTI_DIM& const multi_dimen);
	void deleteMultiDimension();

	//191121 get MBR :Original version,  min id < max id, min value  max value to insert into Rtree. Diiferent from APCA version min id == max id
	template<typename T>
	...
	//191121 get MBR : min id, min value, max id, max value to insert into Rtree.
	...
	//191127 get MBR : min_id == max id. min value, max value to insert into Rtree.
	template<typename T, typename Y>
	...
	//191128 get MBR : min_id == max id. min value, max value to insert into Rtree.
	template<typename T, typename Y, typename U>
	...
	//191129 get MBR : min_id == max id. PLA line into Rtree.
	template<typename T, typename Y>
	void get_cmin_cmax_pla(const int& const time_series_length, DoublyLinkedList<T>& const linked_list, Y& const rtree_rectangle);

	//210906 transfer pla to sapla
	template<typename T, typename Y, typename U>
	void transfer_pla_to_sapla(const T& const width_segment_equal_structure, const Y& const pla, DoublyLinkedList<U>& const linked_list);

	//210906 transfer cheby to sapla
	template<typename T, typename Y, typename U>
	void transfer_cheby_to_sapla(const T& const width_segment_equal_structure, const Y& const cheby, DoublyLinkedList<U>& const linked_list);

	//210906 transfer apca to sapla
	template<typename T, typename Y, typename U>
	void transfer_apca_to_sapla(const T& const size_segment, const Y& const apca, DoublyLinkedList<U>& const linked_list);
	...
	void projectMultiToSingle();
	//191106 optimization of above projectMultiToSingle()
	/*
	paremeter: 3 arguments for y projeciton method in APLA
	*/
	template<typename T, typename Y>
	...
	// build index & KNN
	RTREE& buidRTreeIndex();
	//191108 build Rtree & KNN. Add APLA & ICDE07 
	RTREE& buid_rtree_knn();
	...
	//200901 Optimization KNN. Add APLA & ICDE07 
	template<typename T, typename T1, typename T2, typename T3, typename Y, typename U, typename U1, typename U2>
	void print_result_each_time_series(const U& const input_argument, const vector<T>& const original_time_series_vector, const DoublyLinkedList<Y>& const doubly_linked_list, const T1& const pla, const T2& const apca_paa, const T3& const chebyshev, const U1& const output_argument, const U2& const result_record);
...
// Method:APLA_NODE_PAIR
// Qualifier:  for KNN search in internal node.
// Input: 1 d_dist distance, 2 time seris( APLA point ) id. 3 pointer to APCA point. 4 pointer to R-Tree node.
// Output: 
// date:191112
// author:
//***************************************************************
...
// Method:RTREE_NODE_PAIR
// Qualifier:  for KNN search in internal node.
// Input: 1 d_dist distance, 2 time seris( APLA point ) id. 3 pointer to R-Tree node.
// Output: 
// date:191128
// author:
//***************************************************************
...
void MULTI::transfer_pla_to_sapla(const T& const width_segment_equal_structure, const Y& const pla, DoublyLinkedList<U>& const linked_list) {
	...
	for (int id_segment = 0; id_segment< pla.segmentNum; id_segment++) {
		...
		const int number_segment = id_segment + 1;
		if (id_segment < width_segment_equal_structure.remainder) {
			segment.rectangle_width = width_segment_equal_structure.segment_length_first;
			segment.right_endpoint = number_segment * width_segment_equal_structure.segment_length_first - 1;
		}
		else {
			segment.rectangle_width = width_segment_equal_structure.segment_length_second;
			segment.right_endpoint = width_segment_equal_structure.remainder + number_segment * width_segment_equal_structure.segment_length_second - 1;
		}

		APLA::get_apla_coefficients_segment(segment);

		linked_list.emplace_back(segment);
	}
...
TEMPLATE
template<typename T, typename Y, typename U>
void MULTI::transfer_cheby_to_sapla(const T& const width_segment_equal_structure, const Y& const cheby, DoublyLinkedList<U>& const linked_list) {
	U segment;

	for (int id_segment = 0; id_segment < cheby.segmentNum; id_segment++) {

		segment.apla.a = 0;
		segment.apla.b = cheby.coefficient[id_segment];
...
TEMPLATE
template<typename T, typename Y, typename U>
void MULTI::transfer_apca_to_sapla(const T& const size_segment, const Y& const apca, DoublyLinkedList<U>& const linked_list) {
	assert(apca.segmentNum == size_segment);

	U segment;

	segment.apla.a = 0;
	segment.apla.b = apca.v[0];
	segment.right_endpoint = apca.r[0];
	segment.rectangle_width = segment.right_endpoint + 1;
	APLA::get_apla_coefficients_segment(segment);

	linked_list.emplace_back(segment);

	for (int id_segment = 1; id_segment < size_segment; id_segment++) {

		segment.apla.a = 0;
		segment.apla.b = apca.v[id_segment];
		segment.right_endpoint = apca.r[id_segment];
		segment.rectangle_width = segment.right_endpoint - apca.r[id_segment - 1];
	...
	for (int point_id = 0; point_id < input_argument.point_number; point_id++) {
		...
		/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
			...
			for (int id_segment = 0; id_segment < temp_list.size(); id_segment++) {
				assert(temp_list[id_segment].apla.a == pla_array[point_id].a[id_segment]);
			...
			for (int id_segment = 0; id_segment < temp_list.size(); id_segment++) {
				...
				assert(temp_list[id_segment].apla.b == apca_point_array[point_id].v[id_segment]);
				assert(temp_list[id_segment].right_endpoint == apca_point_array[point_id].r[id_segment]);
				APLA::assert_segment_apla_coefficient(temp_list[id_segment]);
			}
#endif
			switch (input_argument.representation_option) {
			case 3: {//APCA
		...
		/*=================================  CHEBY  =====================================================================*/
			...
			for (int id_segment = 0; id_segment < temp_list.size(); id_segment++) {
				assert(temp_list[id_segment].apla.a == 0);
				assert(temp_list[id_segment].apla.b == chebyshev_array[point_id].coefficient[id_segment]);
			}
#endif
			vector<long double> a_min(chebyshev_MBR.segmentNum, INF);
			copy(chebyshev_MBR.f, chebyshev_MBR.f + int(chebyshev_MBR.segmentNum), a_min.begin());
...
TEMPLATE
template<typename T, typename Y, typename U>
void MULTI::all_knn(typename TOOL::DATA_SOURCE& const data_source, const vector<T>& const query_time_series_vector, T*& query_time_series, const vector<Y>& const multi_y_projection_argument, const std::multiset<pair<U, int>>& const squential_scan_result_set) {
...
// Input:
// Output:
// Notice: Basic KNN, most copy from APCA KNN . KNN time has IO in approxiation part
// date: 191128
// author:
//************************************
TEMPLATE
template<typename T, typename Y, typename U>
std::multiset<pair<double, int>> MULTI::all_knn_multi(typename TOOL::DATA_SOURCE& const data_source, typename TOOL::INPUT_ARGUMENT& const input_argument, const vector<Y>& const multi_y_projection_argument, DataType*& g_query_time_series, const U& const apcaRTree, T& const approximation_array) {
#ifdef _DEBUG
	...
	int i = NULL, j = NULL;
	priority_queue<RTREE_NODE_PAIR<double, int, U::Node>, vector<RTREE_NODE_PAIR<double, int, U::Node>>, MULTI::priorityIncrement<RTREE_NODE_PAIR<double, int, U::Node>>> queue;// <Rtree node, distance>
	RTREE_NODE_PAIR<double, int, U::Node> f_APLA_Root, f_temp_APLA_Pair;// distance, APCA point(original time series) id, APCA point pointer, Rtree sub node pointer
	list<APCA_KNN_QUAL::ORIGINAL_TIME_SERIES_PAIR> temp; // <dist, original time series id>
	list<APCA_KNN_QUAL::ORIGINAL_TIME_SERIES_PAIR> result;// <dist, original time series id>
	std::multiset<pair<double, int>> result_set;//191204 result
	typename APCA_KNN_QUAL::ORIGINAL_TIME_SERIES_PAIR tempOriginalTimeSeriesPair; // <dist, original time series id>
	RTREE_NODE_PAIR<double, int, U::Node> m_temp_queue_top;//
	...
	while (!queue.empty()) {
		...
		for (typename list<APCA_KNN_QUAL::ORIGINAL_TIME_SERIES_PAIR>::iterator plist = temp.begin(); plist != temp.end();) {
			...
			if (input_argument.K == result.size()) {
				...
				//f_temp_APLA_Pair.approximation_pointer = nullptr;
				//f_APLA_Root.approximation_pointer = nullptr;
				priority_queue< RTREE_NODE_PAIR<double, int, U::Node>, vector<RTREE_NODE_PAIR<double, int, U::Node>>, MULTI::priorityIncrement<RTREE_NODE_PAIR<double, int, U::Node>>>().swap(queue);
...
////#endif
			...
			RTREE_NODE_PAIR<double, int, U::Node> temp_apla_pair;
			typename APCA_KNN_QUAL::REGION fs_region_G;
			typename APCA_KNN_QUAL::initialREGION(fs_region_G, input_argument.point_dimension);
			////cout << "regionNum = " << G.regionNum << endl;
			//cout << "================: " << m_temp_queue_top.p_rtree_node->m_count << endl;
...
TEMPLATE
template<typename T, typename Y, typename U>
std::multiset<pair<double, int>> MULTI::all_knn_multi_210511(typename TOOL::DATA_SOURCE& const data_source, typename TOOL::INPUT_ARGUMENT& const input_argument, const vector<Y>& const multi_y_projection_argument, DataType*& g_query_time_series, const U& const apcaRTree, T& const approximation_array) {
	/*..........................................................................................................*/
#ifdef _DEBUG
	assert(input_argument.file_id != INF && input_argument.K <= input_argument.point_number && input_argument.time_series_length != INF && input_argument.point_number != INF && input_argument.arity_d != INF && input_argument.K != INF);
	assert(input_argument.time_series_length == input_argument.point_multi_single_length);
...
TEMPLATE
template<typename T, typename Y, typename U>
std::multiset<pair<double, int>> MULTI::all_knn_multi_210512(typename TOOL::DATA_SOURCE& const data_source, typename TOOL::INPUT_ARGUMENT& const input_argument, const vector<Y>& const multi_y_projection_argument, DataType*& g_query_time_series, const U& const apcaRTree, T& const approximation_array) {
	/*..........................................................................................................*/
#ifdef _DEBUG
	assert(input_argument.file_id != INF && input_argument.K <= input_argument.point_number && input_argument.time_series_length != INF && input_argument.point_number != INF && input_argument.arity_d != INF && input_argument.K != INF);
	assert(input_argument.time_series_length == input_argument.point_multi_single_length);
