...
public:
	...
	struct Node
	{
		...
		inline bool IsLeaf() { return (m_level == 0); } // A leaf, contains data
...
public:
	...
	struct PartitionVars
	{
		...
		int m_branchCount; //the number of total branches in m_ branchBuf
...
void RTREE_PARTITION_QUAL::Insert(const vector<ELEMTYPE>& const a_min, const vector<ELEMTYPE>& const a_max, const DATATYPE& const a_dataId) {
	...
	//branch.m_rect.value_point_min_vector.assign(1024, INF);
	//branch.m_rect.value_point_max_vector.assign(1024, INF);
...
// Input:
// Output: 
// Note: Add approximation point. Add template
// date:210618
// author:
//***************************************************************
RTREE_TEMPLATE
...
void RTREE_PARTITION_QUAL::Insert(const vector<T>& const a_min, const vector<T>& const a_max, const Y& a_dataId, const DoublyLinkedList<U>& const doubly_linked_list, const vector<T1>& const normalized_time_series_vector) {
	...
	//branch.m_rect.list_original_pointer = new DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>;
	//branch.m_rect.list_partition_pointer = new DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>;
	/*----------------------------------------------------------*/
	...
	copy_n(normalized_time_series_vector.begin(), normalized_time_series_vector.size(), branch.m_rect.original_time_series_vector_pointer->begin());
	...
	m_root->rectangle_in_node.linked_list_original_vector.emplace_back(*branch.m_rect.list_original_pointer);
	m_root->rectangle_in_node.linked_list_partition_vector.emplace_back(*branch.m_rect.list_partition_pointer);*/
	/*=========================================================================================*/
	/*========================  210711 vector of pointer ======================================*/
	//m_root->rectangle_in_node.original_time_series_pointer_vector.emplace_back(&branch.m_rect.original_time_series_vector_pointer);//210706
	//m_root->rectangle_in_node.list_original_pointer_vector.emplace_back(branch.m_rect.list_original_pointer);//for each partitioned approximation point
	//m_root->rectangle_in_node.list_partition_pointer_vector.emplace_back(branch.m_rect.list_partition_pointer);//for each partitioned approximation point
	/*=========================================================================================*/
	/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
	/*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
...
void RTREE_PARTITION_QUAL::copy_approximation_point_original(const DoublyLinkedList<T>& const doubly_linked_list, Y& const m_rect) {
	...
	m_rect.list_original_pointer->clear();
	m_rect.list_partition_pointer->clear();

	/*---*/
	//m_rect.list_original_pointer.copy(doubly_linked_list);
	//m_rect.list_partition_pointer.copy(doubly_linked_list);
	/*---*/

	/*---*/
	AREA_COEFFICIENT_RTREE_NODE temp_segment_coefficients;
	for (int id_segment = 0; id_segment < doubly_linked_list.size(); id_segment++) {
		...
		m_rect.list_original_pointer->emplace_back(temp_segment_coefficients);
		m_rect.list_partition_pointer->emplace_back(temp_segment_coefficients);
	}
	/*--*/

	//m_rect.linked_list_original_vector.emplace_back(m_rect.list_original_pointer);// 210623 delete crash
}
...
inline void RTREE_PARTITION_QUAL::copy_rect_original_series_approximation_vector_pointer(const T& const rectangle_original, Y& const m_rect) {
	...
	/*======================================================  210710 vector of vector  ============================================================*/
	// rectangle_original in Leaf Node 
	if (rectangle_original.original_time_series_vector_pointer && (*rectangle_original.original_time_series_vector_pointer)[0] != INF) {
		/*............................................................................................*/
#ifdef _DEBUG
		assert(rectangle_original.original_time_series_pointer_vector.empty());
		assert(rectangle_original.list_original_pointer_vector.empty() && rectangle_original.list_partition_pointer_vector.empty());
#endif
		/*............................................................................................*/

		//m_rect.original_time_series_vector_vector.emplace_back(rectangle_original.original_time_series_vector_pointer);
		m_rect.original_time_series_pointer_vector.emplace_back(rectangle_original.original_time_series_vector_pointer);//210706
		...
		/*========================            210711 vector of pointer        ======================================*/
		m_rect.list_original_pointer_vector.emplace_back(rectangle_original.list_original_pointer);//for each partitioned approximation point
		m_rect.list_partition_pointer_vector.emplace_back(rectangle_original.list_partition_pointer);//for each partitioned approximation point
		/*==========================================================================================================*/
	}
	else {// rectangle_original in Internal Node 
		/*............................................................................................*/
#ifdef _DEBUG
		assert(!rectangle_original.original_time_series_pointer_vector.empty());
		assert(!rectangle_original.list_original_pointer_vector.empty() && !rectangle_original.list_partition_pointer_vector.empty());
		assert(!rectangle_original.original_time_series_vector_pointer && !m_rect.original_time_series_vector_pointer);
		assert_rectangle(rectangle_original);
		assert_rectangle(m_rect);
#endif
		/*............................................................................................*/

		for (int id_pointer = 0; id_pointer < rectangle_original.original_time_series_pointer_vector.size(); id_pointer++) {
			// non duplicate entry
			if (find(m_rect.list_original_pointer_vector.begin(), m_rect.list_original_pointer_vector.end(), rectangle_original.list_original_pointer_vector[id_pointer]) == m_rect.list_original_pointer_vector.end()) {
				m_rect.original_time_series_pointer_vector.emplace_back(rectangle_original.original_time_series_pointer_vector[id_pointer]);
				...
				m_rect.list_original_pointer_vector.emplace_back(rectangle_original.list_original_pointer_vector[id_pointer]);
				m_rect.list_partition_pointer_vector.emplace_back(rectangle_original.list_partition_pointer_vector[id_pointer]);
			}
		}
		/*========================  210711 vector of pointer ======================================*/
		//copy_ n(rectangle_original.original_time_series_pointer_vector.begin(), rectangle_original.original_time_series_pointer_vector.size(), m_rect.original_time_series_pointer_vector.end());
		//for each partitioned approximation point
		//copy_n (rectangle_original.list_original_pointer_vector.begin(), rectangle_original.list_original_pointer_vector.size(), m_rect.list_original_pointer_vector.end());
		//for each partitioned approximation point
		//copy_n (rectangle_original.list_partition_pointer_vector.begin(), rectangle_original.list_partition_pointer_vector.size(), m_rect.list_partition_pointer_vector.end());
		/*=========================================================================================*/
		/*............................................................................................*/
#ifdef _DEBUG
		assert_rectangle(m_rect);
	...
	//m_rect.linked_list_original_vector.emplace_back(*rectangle_original.list_original_pointer);
	//m_rect.linked_list_partition_vector.emplace_back(*rectangle_original.list_partition_pointer);
	/*=========================================================================================*/
...
void RTREE_PARTITION_QUAL::erase_rectangle_from_sub_rectangle(T& const rectangle_sub, Y& const rectangle) {
	...
	//assert(rectangle_sub.size_entry()<= rectangle.size_entry());
#endif
	/*......................................*/

	for (int id_entry = 0; id_entry < rectangle_sub.size_entry(); id_entry++) {
		rectangle.original_time_series_pointer_vector.erase(std::remove(rectangle.original_time_series_pointer_vector.begin(), rectangle.original_time_series_pointer_vector.end(), rectangle_sub.original_time_series_pointer_vector[id_entry]), rectangle.original_time_series_pointer_vector.end());
		...
		rectangle.list_original_pointer_vector.erase(std::remove(rectangle.list_original_pointer_vector.begin(), rectangle.list_original_pointer_vector.end(), rectangle_sub.list_original_pointer_vector[id_entry]), rectangle.list_original_pointer_vector.end());
		rectangle.list_partition_pointer_vector.erase(std::remove(rectangle.list_partition_pointer_vector.begin(), rectangle.list_partition_pointer_vector.end(), rectangle_sub.list_partition_pointer_vector[id_entry]), rectangle.list_partition_pointer_vector.end());
	}
...
inline void RTREE_PARTITION_QUAL::free_shared_ptr_linked_list(shared_ptr<T>& const smart_pointer) {
	if (smart_pointer != nullptr) {
		smart_pointer->clear();
		smart_pointer.reset();
		//a_node->m_branch[index].m_rect.list_original_pointer = nullptr;
		/* 210709 delete a_node->m_branch[index].m_rect.list_original_pointer;
		a_node->m_branch[index].m_rect.list_original_pointer = nullptr;*/
	}
}
...
void RTREE_PARTITION_QUAL::FreeNode(Node* a_node)
{
	...
	/*--------------------------- for one whole approximation ---------------------------------*/
	free_shared_ptr_linked_list(a_node->rectangle_in_node.list_partition_global_min_pointer);
	free_shared_ptr_linked_list(a_node->rectangle_in_node.list_partition_global_max_pointer);
	/*-----------------------------------------------------------------------------------------*/

	/*-------------- for each segment approximation that has max differnce ---------------------*/
	free_shared_ptr_linked_list(a_node->rectangle_in_node.list_partition_local_min_pointer);
	free_shared_ptr_linked_list(a_node->rectangle_in_node.list_partition_local_max_pointer);
	/*------------------------------------------------------------------------------------------*/

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~ partition 210701 clear pointer of approxiation points ~~~~~~~~~~~~~~~~~~~~~*/
	free_shared_ptr_linked_list(a_node->rectangle_in_node.list_original_pointer);
	free_shared_ptr_linked_list(a_node->rectangle_in_node.list_partition_pointer);
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	for (int index = 0; index < a_node->m_count; ++index) {
		...
		/*--------------------------- for one whole approximation ---------------------------------*/
		free_shared_ptr_linked_list(a_node->m_branch[index].m_rect.list_partition_global_min_pointer);
		free_shared_ptr_linked_list(a_node->m_branch[index].m_rect.list_partition_global_max_pointer);
		/*-----------------------------------------------------------------------------------------*/

		/*-------------- for each segment approximation that has max differnce ---------------------*/
		free_shared_ptr_linked_list(a_node->m_branch[index].m_rect.list_partition_local_min_pointer);
		free_shared_ptr_linked_list(a_node->m_branch[index].m_rect.list_partition_local_max_pointer);
		/*------------------------------------------------------------------------------------------*/

		/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

		/*~~~~~~~~~~~~~~~~~~~~~~~~~~ partition 210701 clear pointer of approxiation points ~~~~~~~~~~~~~~~~~~~~~*/
		free_shared_ptr_linked_list(a_node->m_branch[index].m_rect.list_original_pointer);
		free_shared_ptr_linked_list(a_node->m_branch[index].m_rect.list_partition_pointer);

		//if (a_node->m_branch[index].m_rect.list_original_pointer != nullptr) {
		//	a_node->m_branch[index].m_rect.list_original_pointer->clear();
		//	a_node->m_branch[index].m_rect.list_original_pointer.reset();
		//	//a_node->m_branch[index].m_rect.list_original_pointer = nullptr;
		//	/* 210709 delete a_node->m_branch[index].m_rect.list_original_pointer;
		//	a_node->m_branch[index].m_rect.list_original_pointer = nullptr;*/
		//}
		//
		//if (a_node->m_branch[index].m_rect.list_partition_pointer != nullptr) {
		//	a_node->m_branch[index].m_rect.list_partition_pointer->clear();
		//	a_node->m_branch[index].m_rect.list_partition_pointer.reset();
		//	//a_node->m_branch[index].m_rect.list_partition_pointer = nullptr;
		//	/* 210709delete a_node->m_branch[index].m_rect.list_partition_pointer;
		//	a_node->m_branch[index].m_rect.list_partition_pointer = nullptr;*/
		//}
		/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	}
...
inline T& RTREE_PARTITION_QUAL::allocate_rectangle_entry(T& const a_rect) { //210804 allocatge new memory to pointer or vector for entry
	...
	a_rect.original_time_series_vector_pointer = std::make_shared<vector<long double>>();
	//a_rect.original_time_series_vector_pointer->assign(1024, INF);
	...
	a_rect.list_original_pointer = std::make_shared<DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>>();
	a_rect.list_partition_pointer = std::make_shared<DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>>();
	/*a_rect.list_original_pointer = new DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>;
	a_rect.list_partition_pointer = new DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>;*/
...
inline T& RTREE_PARTITION_QUAL::allocate_rectangle_node(T& const a_rect) { //210804 allocatge new memory to pointer or vector for node:
	...
	/*~~~~~~~~~~~~~~~~~~~   210716 Volume   ~~~~~~~~~~~~~~~~~~~~~~~~*/
	a_rect.volume_minmax_segment = -INF;// max semgent difference
	a_rect.volume_minmax_segment_sqrt = -INF;// max segment difference square root
	a_rect.volume_minmax_whole = -INF;// max whole difference
	a_rect.volume_minmax_whole_sqrt = -INF;// max whole difference square root

	a_rect.volume_minmax_point = -INF;// max reconstruct point difference
	a_rect.volume_minmax_point_sqrt = -INF; // max reconstruct point difference square root
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~    210718 for rectangle in node     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

	/*--------------------------- max(at+b)-min(at+b) ---------------------------------*/
	//a_rect.value_point_min_vector.assign(1024, INF);
	//a_rect.value_point_max_vector.assign(1024, -INF);
	/*----------------------------------------------------------------------------------*/

	/*--------------------------- for one whole approximation ---------------------------------*/
	a_rect.seed_0 = (std::numeric_limits<int>::min)();
	a_rect.seed_1 = (std::numeric_limits<int>::min)();
	a_rect.list_partition_global_min_pointer = std::make_shared<DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>>();
	a_rect.list_partition_global_max_pointer = std::make_shared<DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>>();
	/*-----------------------------------------------------------------------------------------*/

	/*-------------- for each segment approximation that has max differnce ---------------------*/
	a_rect.list_partition_local_min_pointer = std::make_shared<DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>>();
	a_rect.list_partition_local_max_pointer = std::make_shared<DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>>();
	/*------------------------------------------------------------------------------------------*/
...
inline T& RTREE_PARTITION_QUAL::clear_vector_in_rectangle(T& const a_rect) {
	...
	/*--------------------------- max(at+b)-min(at+b) ---------------------------------*/
	/*a_rect.value_point_min_vector.clear();
	a_rect.value_point_min_vector.shrink_to_fit();
	a_rect.value_point_max_vector.clear();
	a_rect.value_point_max_vector.shrink_to_fit();*/
	/*----------------------------------------------------------------------------------*/
	...
	/*--------------------------- for one whole approximation ---------------------------------*/
	a_rect.list_partition_global_min_pointer->clear();
	a_rect.list_partition_global_max_pointer->clear();
	/*free_shared_ptr_linked_list(a_rect.list_partition_global_min_pointer);
	free_shared_ptr_linked_list(a_rect.list_partition_global_max_pointer);*/
	/*-----------------------------------------------------------------------------------------*/

	/*-------------- for each segment approximation that has max differnce ---------------------*/
	a_rect.id_branch_segment_min_vector.clear();
	a_rect.id_branch_segment_min_vector.shrink_to_fit();

	a_rect.id_branch_segment_max_vector.clear();
	a_rect.id_branch_segment_max_vector.shrink_to_fit();

	a_rect.difference_each_segment_max_vector.clear();
	a_rect.difference_each_segment_max_vector.shrink_to_fit();

	a_rect.difference_power_each_segment_max_vector.clear();
	a_rect.difference_power_each_segment_max_vector.shrink_to_fit();

	a_rect.list_partition_local_min_pointer->clear();
	a_rect.list_partition_local_max_pointer->clear();
	/*free_shared_ptr_linked_list(a_rect.list_partition_local_min_pointer);
	free_shared_ptr_linked_list(a_rect.list_partition_local_max_pointer);*/
	/*------------------------------------------------------------------------------------------*/
	...
	/*---------------    210710 shared pointer    ------------------------*/
	a_rect.original_time_series_pointer_vector.clear();
	a_rect.original_time_series_pointer_vector.shrink_to_fit();
	//a_rect.reconstruct_time_series_pointer_vector.clear();
	//a_rect.reconstruct_time_series_pointer_vector.shrink_to_fit();
	a_rect.list_original_pointer_vector.clear();
	a_rect.list_original_pointer_vector.shrink_to_fit();
	a_rect.list_partition_pointer_vector.clear();
	a_rect.list_partition_pointer_vector.shrink_to_fit();

	/*free_pointer_vector(a_rect.original_time_series_pointer_vector);
	free_pointer_vector(a_rect.list_original_pointer_vector);
	free_pointer_vector(a_rect.list_partition_pointer_vector);*/
	/*--------------------------------------------------------------------*/
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
...
inline T& RTREE_PARTITION_QUAL::free_rectangle(T& const a_rect) {//210730 free memory to pointer or vectotr
	...
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~    210718 for rectangle in branch     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	if (a_rect.original_time_series_vector_pointer) {
		a_rect.original_time_series_vector_pointer->clear();
		a_rect.original_time_series_vector_pointer->shrink_to_fit();
		free_shared_ptr_linked_list(a_rect.original_time_series_vector_pointer);
	}
	...
	free_shared_ptr_linked_list(a_rect.list_original_pointer);
	free_shared_ptr_linked_list(a_rect.list_partition_pointer);
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
inline T& RTREE_PARTITION_QUAL::clear_rectangle_PartitionVars(T& const a_rect) {
	...
	a_rect.list_partition_global_min_pointer.reset();
	a_rect.list_partition_global_max_pointer.reset();
	a_rect.list_partition_local_min_pointer.reset();
	a_rect.list_partition_local_max_pointer.reset();
...
inline void RTREE_PARTITION_QUAL::AllocBranch(T& const a_branch) {
	...
	///*=================================== partition 210705 =========================================================*/
	//a_branch.m_rect.value_point_min_vector.assign(1024, INF);
	//a_branch.m_rect.value_point_max_vector.assign(1024, INF);
	...
	//a_branch.m_rect.list_original_pointer = new DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>;
	//a_branch.m_rect.list_partition_pointer = new DoublyLinkedList<AREA_COEFFICIENT_RTREE_NODE>;
	///*===========================================================================================================*/
}
...
bool RTREE_PARTITION_QUAL::InsertRectRec(Branch& a_branch, Node* a_node, Node** a_newNode, int a_level) {
	...
	else if (a_node->m_level == a_level) {// this is leaf node->contain entry.  In insert function a_level == 0
...
RTREE_TEMPLATE
bool RTREE_PARTITION_QUAL::InsertRect(Branch& a_branch, Node** a_root, int a_level)
{
	ASSERT(a_root);
...
bool RTREE_PARTITION_QUAL::AddBranch(Branch* a_branch, Node* a_node, Node** a_newNode) {
	...
	if (a_node->m_count < TMAXNODES) {  // Split a_node won't be necessary, a_newNode is nullptr
		...
		case 8: {// Initial 200706
			...
			a_node->m_branch[a_node->m_count - 1].branch_to_parent_node_pointer = a_node;

			if (a_node->m_branch[a_node->m_count - 1].m_child) {
			...
			if (a_node->m_count > 1 && a_node->IsLeaf()) {
				...
				/**/
				OPTION_DISTANCE_STRUCT<int, int, int> option_distance_struct(0, 0, 0);
				switch (representation_option) {
				case 3: {//APCA
					option_distance_struct.option_method = 1;
					break;
				}
				default:
					break;
				...
				for (int id_branch = 0; id_branch < a_node->m_count - 1; id_branch++) {
					APLA::get_same_partition_SAPLA_limit_only_move_1(*a_node->m_branch[a_node->m_count - 1].m_rect.original_time_series_vector_pointer, *a_node->m_branch[id_branch].m_rect.original_time_series_vector_pointer, *a_node->m_branch[a_node->m_count - 1].m_rect.list_partition_pointer, *a_node->m_branch[id_branch].m_rect.list_partition_pointer, option_distance_struct);
					//APLA::get_same_partition_SAPLA_210818(*a_node->m_branch[id_branch].m_rect.original_time_series_vector_pointer, *a_node->m_branch[a_node->m_count - 1].m_rect.original_time_series_vector_pointer, *a_node->m_branch[id_branch].m_rect.list_partition_pointer, *a_node->m_branch[a_node->m_count - 1].m_rect.list_partition_pointer, number_points);
					//APLA::get_same_partition_SAPLA_debug(*a_node->m_branch[id_branch].m_rect.original_time_series_vector_pointer, *a_node->m_branch[a_node->m_count - 1].m_rect.original_time_series_vector_pointer, *a_node->m_branch[id_branch].m_rect.list_partition_pointer, *a_node->m_branch[a_node->m_count - 1].m_rect.list_partition_pointer);
					
					/*--------------------210817 So far not need reconstructed time series---------*/
					//APLA::getAPLAReconstructSeries(*a_node->m_branch[id_branch].m_rect.list_partition_pointer, *a_node->m_branch[id_branch].m_rect.reconstruct_time_series_vector_pointer);
					//APLA::getAPLAReconstructSeries(*a_node->m_branch[a_node->m_count - 1].m_rect.list_partition_pointer, *a_node->m_branch[a_node->m_count - 1].m_rect.reconstruct_time_series_vector_pointer);
					/*------------------------------------------------------------------------------*/
			...
			/*210711*/
			//a_node->rectangle_in_node.linked_list_partition_vector.emplace_back(*a_node->m_branch[a_node->m_count - 1].m_rect.list_partition_pointer);
			break;
		}
		default:
			break;
		...
		case 8: {// Initial 200706
			if (a_node->IsLeaf()) {
				...
				/*......................................*/
#ifdef _DEBUG
				assert(a_branch->m_rect.list_partition_pointer);
#endif
				/*......................................*/
				for (int id_branch = 0; id_branch < a_node->m_count; id_branch++) {
					APLA::get_same_partition_SAPLA_limit_only_move_1(*a_branch->m_rect.original_time_series_vector_pointer, *a_node->m_branch[id_branch].m_rect.original_time_series_vector_pointer, *a_branch->m_rect.list_partition_pointer, *a_node->m_branch[id_branch].m_rect.list_partition_pointer, option_distance_struct);
					//APLA::get_same_partition_SAPLA_210818(*a_node->m_branch[id_branch].m_rect.original_time_series_vector_pointer, *a_branch->m_rect.original_time_series_vector_pointer, *a_node->m_branch[id_branch].m_rect.list_partition_pointer, *a_branch->m_rect.list_partition_pointer, number_points);
					//APLA::get_same_partition_SAPLA_debug(*a_node->m_branch[id_branch].m_rect.original_time_series_vector_pointer, *a_branch->m_rect.original_time_series_vector_pointer, *a_node->m_branch[id_branch].m_rect.list_partition_pointer, *a_branch->m_rect.list_partition_pointer);
					/*--------------------210817 So far not need reconstructed time series---------*/
					//APLA::getAPLAReconstructSeries(*a_node->m_branch[id_branch].m_rect.list_partition_pointer, *a_node->m_branch[id_branch].m_rect.reconstruct_time_series_vector_pointer);
					//APLA::getAPLAReconstructSeries(*a_branch->m_rect.list_partition_pointer, *a_branch->m_rect.reconstruct_time_series_vector_pointer);
					/*-----------------------------------------------------------------------------*/
					//partition_rect_volume(a_node->m_branch[id_branch].m_rect, a_node->rectangle_in_node);
				}
...
int RTREE_PARTITION_QUAL::pick_branch(T& const a_rect, Y& const a_node) {
	...
	for (int id_branch = 0; id_branch < a_node.m_count; ++id_branch) {
		...
		volume_current = a_node.m_branch[id_branch].m_rect.volume_minmax_whole_sqrt;
...
long double RTREE_PARTITION_QUAL::combine_compute_partition_rectangle_by_entry(T& const rectangle_entry, Y& const rectangle_node, U& const rectangle_combine) {
	...
	/*........................................................................................*/
#ifdef _DEBUG
	assert(rectangle_combine.size_entry() == 1 + rectangle_node.size_entry());
#endif
	/*........................................................................................*/
	...
	for (int id_entry = 0; id_entry < rectangle_node.size_entry(); id_entry++) {
		APLA::get_same_partition_SAPLA_limit_only_move_1(*rectangle_combine.original_time_series_pointer_vector[rectangle_node.size_entry()], *rectangle_combine.original_time_series_pointer_vector[id_entry], *rectangle_combine.list_partition_pointer_vector[rectangle_node.size_entry()], *rectangle_combine.list_partition_pointer_vector[id_entry], option_distance_struct);
		//APLA::get_same_partition_SAPLA_210818(*rectangle_combine.original_time_series_pointer_vector[id_entry], *rectangle_combine.original_time_series_pointer_vector[rectangle_node.size_entry()], *rectangle_combine.list_partition_pointer_vector[id_entry], *rectangle_combine.list_partition_pointer_vector[rectangle_node.size_entry()], number_points);
		//APLA::get_same_partition_SAPLA_debug(*rectangle_combine.original_time_series_pointer_vector[id_entry], *rectangle_combine.original_time_series_pointer_vector[rectangle_node.size_entry()], *rectangle_combine.list_partition_pointer_vector[id_entry], *rectangle_combine.list_partition_pointer_vector[rectangle_node.size_entry()]);
		/*--------------------210817 So far not need reconstructed time series---------*/
		//APLA::getAPLAReconstructSeries(*rectangle_combine.list_partition_pointer_vector[id_entry], *rectangle_combine.reconstruct_time_series_pointer_vector[id_entry]);
		//APLA::getAPLAReconstructSeries(*rectangle_combine.list_partition_pointer_vector[rectangle_node.size_entry()], *rectangle_combine.reconstruct_time_series_pointer_vector[rectangle_node.size_entry()]);
	    /*----------------------------------------------------------------------------*/
	}
	...
	assert_rectangle(rectangle_node);
	//assert_rectangle(rectangle_combine);
	/*if (rectangle_node.list_original_pointer_vector.size() > 1) {
		long double distance_1 = get_sqrt_distance_sapla_same_endpoints(*rectangle_entry.list_partition_pointer, *rectangle_node.list_partition_global_min_pointer);
		long double distance_2 = get_sqrt_distance_sapla_same_endpoints(*rectangle_entry.list_partition_pointer, *rectangle_node.list_partition_global_max_pointer);
		long double distance_max_0 = max(distance_1, distance_2);
		long double distance_max_1 = max(rectangle_node.volume_minmax_whole_sqrt, distance_max_0);
		assert(rectangle_combine.volume_minmax_whole_sqrt == distance_max_1);
	}*/
#endif
	/*......................................*/
...
long double RTREE_PARTITION_QUAL::combine_compute_partition_rectangle_by_entry_210905(T& const rectangle_entry, Y& const rectangle_node) {
	...
	APLA::OPTION_DISTANCE_STRUCT<int, int, int> option_distance_struct(0, 0, 0);
	switch (representation_option) {
	case 3: {//APCA
		option_distance_struct.option_method = 1;
		break;
	...
	const int size_entry = rectangle_node.size_entry();

	for (int id_entry = 0; id_entry < size_entry; id_entry++) {
		distance_sqrt_SAPLA_with_new_entry_current = APLA::get_same_partition_SAPLA_limit_only_move_1(*rectangle_entry.original_time_series_vector_pointer, *rectangle_node.original_time_series_pointer_vector[id_entry], *rectangle_entry.list_partition_pointer, *rectangle_node.list_partition_pointer_vector[id_entry], option_distance_struct);
...
//	assert(rectangle_new.original_time_series_vector_pointer->size() == node->rectangle_in_node.value_point_min_vector.size() && node->rectangle_in_node.value_point_min_vector.size() == node->rectangle_in_node.value_point_max_vector.size());
...
//		node->rectangle_in_node.value_point_max_vector[id_point] = max(node->rectangle_in_node.value_point_max_vector[id_point], (*rectangle_new.reconstruct_time_series_vector_pointer)[id_point]);
//		node->rectangle_in_node.value_point_min_vector[id_point] = min(node->rectangle_in_node.value_point_min_vector[id_point], (*rectangle_new.reconstruct_time_series_vector_pointer)[id_point]);
//	}
//
//	if (node->rectangle_in_node.list_partition_local_min_pointer->size() == rectangle_new.list_partition_pointer->size()) {
//
//		/*...........................................................................................................................................................*/
//#ifdef _DEBUG
//		assert(node->rectangle_in_node.list_partition_local_min_pointer->size() == node->rectangle_in_node.list_partition_local_max_pointer->size() && node->rectangle_in_node.list_partition_global_min_pointer->size() == node->rectangle_in_node.list_partition_global_max_pointer->size());
//		assert(node->rectangle_in_node.list_partition_local_min_pointer->size() == node->rectangle_in_node.list_partition_global_min_pointer->size());
//		assert(node->rectangle_in_node.list_partition_local_min_pointer->size() == rectangle_new.list_partition_pointer->size());
//#endif
//		/*...........................................................................................................................................................*/
//	}
//	else {
//		node->rectangle_in_node.list_partition_local_min_pointer->clear();
//		node->rectangle_in_node.list_partition_local_max_pointer->clear();
//		node->rectangle_in_node.list_partition_global_min_pointer->clear();
//		node->rectangle_in_node.list_partition_global_max_pointer->clear();
//	}
//
//	/*...................................................................*/
//#ifdef _DEBUG
...
Y& RTREE_PARTITION_QUAL::scan_each_node_for_variant(T& const node, Y& const rectangle) {
	...
	if (node.IsInternalNode()) {  // Internal node
		...
		/*.............................*/
#ifdef _DEBUG
		assert(node.m_count == node.rectangle_in_node.original_time_series_pointer_vector.size());
		assert(node.IsLeaf());
#endif
		/*.............................*/

		for (int id_branch = 0; id_branch < node.m_count; id_branch++) {
			rectangle.original_time_series_pointer_vector.emplace(node.m_branch[id_branch].m_rect.original_time_series_vector_pointer);
		}
	}
...
long double& RTREE_PARTITION_QUAL::get_convex_hull_volume_by_minmax_reconstruct_point(const T* const branch_pointer, const Y& const size_branch, U& const rectangle_combined) {
	...
	/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  Min Max Point  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
	/*--------------------------- max(at+b)-min(at+b) ---------------------------------*/
	rectangle_combined.value_point_min_vector.assign(1024, INF);
	rectangle_combined.value_point_max_vector.assign(1024, -INF);
	/*----------------------------------------------------------------------------------*/

	rectangle_combined.volume_minmax_point = 0;// max reconstruct point difference
	rectangle_combined.volume_minmax_point_sqrt = 0;// max reconstruct point difference square root

	for (int id_point = 0; id_point < branch_pointer[0].m_rect.original_time_series_vector_pointer->size(); id_point++) {

		for (int id_branch = 0; id_branch < size_branch; id_branch++) {
			/*...........................................................................................................................................................*/
#ifdef _DEBUG
			assert(branch_pointer[id_branch].m_rect.list_partition_pointer->size() >= branch_pointer[id_branch].m_rect.list_original_pointer->size());
			...
			assert(rectangle_combined.value_point_max_vector.size() == rectangle_combined.value_point_min_vector.size());
			//assert(rectangle_combined.value_point_max_vector.size() == branch_pointer[id_branch].m_rect.reconstruct_time_series_vector_pointer->size());
			assert(branch_pointer[id_branch].m_rect.original_time_series_vector_pointer->size() - 1 == branch_pointer[id_branch].m_rect.list_partition_pointer->back().right_endpoint);
			APLA::assert_has_same_endpoints(*branch_pointer[0].m_rect.list_partition_pointer, *branch_pointer[id_branch].m_rect.list_partition_pointer);
#endif
			/*...........................................................................................................................................................*/

			//rectangle_combined.value_point_max_vector[id_point] = max(rectangle_combined.value_point_max_vector[id_point], (*branch_pointer[id_branch].m_rect.reconstruct_time_series_vector_pointer)[id_point]);
			//rectangle_combined.value_point_min_vector[id_point] = min(rectangle_combined.value_point_min_vector[id_point], (*branch_pointer[id_branch].m_rect.reconstruct_time_series_vector_pointer)[id_point]);
		}

		/*...........................................................................................................................................................*/
#ifdef _DEBUG
		assert(rectangle_combined.value_point_max_vector[id_point] >= rectangle_combined.value_point_min_vector[id_point]);
#endif
		/*...........................................................................................................................................................*/

		const long double difference_point = rectangle_combined.value_point_max_vector[id_point] - rectangle_combined.value_point_min_vector[id_point];
		rectangle_combined.volume_minmax_point += difference_point;
		rectangle_combined.volume_minmax_point_sqrt += powl(difference_point, 2);
	}
	...
	return rectangle_combined.volume_minmax_point_sqrt = sqrtl(rectangle_combined.volume_minmax_point_sqrt);
	/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
}
...
long double& RTREE_PARTITION_QUAL::get_convex_hull_by_minmax_reconstruct_point(T& const rectangle_in_node) {
	...
	/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  Min Max Point  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
	/*--------------------------- max(at+b)-min(at+b) ---------------------------------*/
	rectangle_in_node.value_point_min_vector.assign(1024, INF);
	rectangle_in_node.value_point_max_vector.assign(1024, -INF);
	/*----------------------------------------------------------------------------------*/

	rectangle_in_node.volume_minmax_point = 0;// max reconstruct point difference
	rectangle_in_node.volume_minmax_point_sqrt = 0;// max reconstruct point difference square root

	for (int id_point = 0; id_point < rectangle_in_node.value_point_min_vector.size(); id_point++) {

		for (int id_branch = 0; id_branch < rectangle_in_node.list_original_pointer_vector.size(); id_branch++) {
			/*...........................................................................................................................................................*/
#ifdef _DEBUG
			assert(rectangle_in_node.value_point_max_vector.size() == rectangle_in_node.value_point_min_vector.size());
			assert(rectangle_in_node.original_time_series_pointer_vector[0]->size() - 1 == rectangle_in_node.list_partition_pointer_vector[0]->back().right_endpoint);
			APLA::assert_has_same_endpoints(*rectangle_in_node.list_partition_pointer_vector[0], *rectangle_in_node.list_partition_pointer_vector[id_branch]);
#endif
			/*...........................................................................................................................................................*/

			//rectangle_in_node.value_point_max_vector[id_point] = max(rectangle_in_node.value_point_max_vector[id_point], (*rectangle_in_node.reconstruct_time_series_pointer_vector[id_branch])[id_point]);
			//rectangle_in_node.value_point_min_vector[id_point] = min(rectangle_in_node.value_point_min_vector[id_point], (*rectangle_in_node.reconstruct_time_series_pointer_vector[id_branch])[id_point]);
		}

		/*...........................................................................................................................................................*/
#ifdef _DEBUG
		assert(rectangle_in_node.value_point_max_vector[id_point] >= rectangle_in_node.value_point_min_vector[id_point]);
#endif
		/*...........................................................................................................................................................*/

		const long double difference_point = rectangle_in_node.value_point_max_vector[id_point] - rectangle_in_node.value_point_min_vector[id_point];
		rectangle_in_node.volume_minmax_point += difference_point;
		rectangle_in_node.volume_minmax_point_sqrt += powl(difference_point, 2);
	}
	...
	return rectangle_in_node.volume_minmax_point_sqrt = sqrtl(rectangle_in_node.volume_minmax_point_sqrt);
	/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
...
long double& RTREE_PARTITION_QUAL::get_convex_hull_volume_by_distance_approximation_global(const T* const branch_pointer, const Y& const size_branch, U& const rectangle_combined) {
...
#ifdef _DEBUG
	assert(size_branch > 0);
	APLA::assert_has_same_endpoints(*rectangle_combined.list_partition_global_min_pointer, *rectangle_combined.list_partition_global_max_pointer);
	assert_branch_rect_in_node(rectangle_combined, branch_pointer);
#endif
	/*...........................................................................................................................................................*/

	rectangle_combined.volume_minmax_whole_sqrt = rectangle_combined.volume_minmax_whole = -INF;
	...
	for (int id_branch_a = 0; id_branch_a < size_branch - 1; ++id_branch_a) {
		for (int id_branch_b = id_branch_a + 1; id_branch_b < size_branch; ++id_branch_b) {
			/*...........................................................................................................................................................*/
#ifdef _DEBUG
			APLA::assert_has_same_endpoints(*branch_pointer[id_branch_a].m_rect.list_partition_pointer, *branch_pointer[id_branch_b].m_rect.list_partition_pointer);
#endif
			/*...........................................................................................................................................................*/

			distance_current = APLA::get_sqrt_distance_sapla_same_endpoints(*branch_pointer[id_branch_a].m_rect.list_partition_pointer, *branch_pointer[id_branch_b].m_rect.list_partition_pointer);
			...
			if (distance_current > rectangle_combined.volume_minmax_whole_sqrt) {

				rectangle_combined.volume_minmax_whole_sqrt = distance_current;
				rectangle_combined.volume_minmax_whole = powl(rectangle_combined.volume_minmax_whole_sqrt, 2);
	...
	rectangle_combined.list_partition_global_min_pointer->copy(*branch_pointer[rectangle_combined.seed_0].m_rect.list_partition_pointer);
	rectangle_combined.list_partition_global_max_pointer->copy(*branch_pointer[rectangle_combined.seed_1].m_rect.list_partition_pointer);
	/*---------------------------------------------------------------------------*/
	...
	return rectangle_combined.volume_minmax_whole_sqrt;
}
...
long double& RTREE_PARTITION_QUAL::get_convex_hull_by_distance_approximation_global(T& const rectangle_in_node) {
	/*...........................................................................................................................................................*/
#ifdef _DEBUG
	APLA::assert_has_same_endpoints(*rectangle_in_node.list_partition_global_min_pointer, *rectangle_in_node.list_partition_global_max_pointer);
	assert_rectangle(rectangle_in_node);
#endif
	/*...........................................................................................................................................................*/

	rectangle_in_node.volume_minmax_whole_sqrt = rectangle_in_node.volume_minmax_whole = -INF;
	long double distance_current = -INF;
	rectangle_in_node.seed_0 = rectangle_in_node.seed_1 = -1;

	/*====================================   For global approximation. Bubble sort: compare every two of branches  =================================================*/
	for (int id_branch_a = 0; id_branch_a < rectangle_in_node.list_original_pointer_vector.size() - 1; ++id_branch_a) {
		for (int id_branch_b = id_branch_a + 1; id_branch_b < rectangle_in_node.list_original_pointer_vector.size(); ++id_branch_b) {
			/*...........................................................................................................................................................*/
#ifdef _DEBUG
			APLA::assert_has_same_endpoints(*rectangle_in_node.list_partition_pointer_vector[id_branch_a], *rectangle_in_node.list_partition_pointer_vector[id_branch_b]);
#endif
			/*...........................................................................................................................................................*/

			distance_current = APLA::get_sqrt_distance_sapla_same_endpoints(*rectangle_in_node.list_partition_pointer_vector[id_branch_a], *rectangle_in_node.list_partition_pointer_vector[id_branch_b]);

			//CombineRect(&a_parVars->m_branchBuf[id_branch_a].m_rect, &a_parVars->m_branchBuf[id_branch_b].m_rect, &oneRect);
			//waste = CalcRectVolume(&oneRect) - area[id_branch_a] - area[id_branch_b];

			//cout << "*******waste: " << waste << endl;
			if (distance_current > rectangle_in_node.volume_minmax_whole_sqrt) {

				rectangle_in_node.volume_minmax_whole_sqrt = distance_current;
				rectangle_in_node.volume_minmax_whole = powl(rectangle_in_node.volume_minmax_whole_sqrt, 2);
	...
	/*------------  get two approximation with max SAPLA distance  -------------*/
	rectangle_in_node.list_partition_global_min_pointer->copy(*rectangle_in_node.list_partition_pointer_vector[rectangle_in_node.seed_0]);
	rectangle_in_node.list_partition_global_max_pointer->copy(*rectangle_in_node.list_partition_pointer_vector[rectangle_in_node.seed_1]);
	/*---------------------------------------------------------------------------*/
	...
	return rectangle_in_node.volume_minmax_whole_sqrt;
}
...
long double& RTREE_PARTITION_QUAL::get_convex_hull_volume_by_distance_approximation_local(const T* const branch_pointer, const Y& const size_branch, U& const rectangle_combined) {
...
#ifdef _DEBUG
	assert(size_branch > 0);
	if (rectangle_combined.list_partition_local_min_pointer->size() == branch_pointer[0].m_rect.list_partition_pointer->size()) {
		APLA::assert_has_same_endpoints(*rectangle_combined.list_partition_local_min_pointer, *branch_pointer[0].m_rect.list_partition_pointer);
	}
	else {
		assert(rectangle_combined.list_partition_local_min_pointer->empty() && !branch_pointer[0].m_rect.list_partition_pointer->empty());
	}
	//assert_branch_rect_in_node(rectangle_combined, branch_pointer);
#endif
	/*...........................................................................................................................................................*/
	long double distance_power_segment_current = -INF;
	rectangle_combined.difference_each_segment_max_vector.assign(branch_pointer[0].m_rect.list_partition_pointer->size(), -INF);
	rectangle_combined.difference_power_each_segment_max_vector.assign(branch_pointer[0].m_rect.list_partition_pointer->size(), -INF);
	rectangle_combined.id_branch_segment_min_vector.assign(branch_pointer[0].m_rect.list_partition_pointer->size(), -1);
	rectangle_combined.id_branch_segment_max_vector.assign(branch_pointer[0].m_rect.list_partition_pointer->size(), -1);

	for (int id_segment = 0; id_segment < branch_pointer[0].m_rect.list_partition_pointer->size(); id_segment++) {
		...
		for (int id_branch_a = 0; id_branch_a < size_branch - 1; ++id_branch_a) {
			for (int id_branch_b = id_branch_a + 1; id_branch_b < size_branch; ++id_branch_b) {
				...
				/*...........................................................................................................................................................*/
#ifdef _DEBUG
				assert(rectangle_combined.difference_power_each_segment_max_vector.size() == rectangle_combined.difference_each_segment_max_vector.size());
				APLA::assert_has_same_endpoints(*branch_pointer[id_branch_a].m_rect.list_partition_pointer, *branch_pointer[id_branch_b].m_rect.list_partition_pointer);
				APLA::assert_has_same_endpoints(*rectangle_combined.list_partition_local_min_pointer, *rectangle_combined.list_partition_local_max_pointer);
#endif
				/*...........................................................................................................................................................*/

				distance_power_segment_current = APLA::get_segment_distance_LB(branch_pointer[id_branch_a].m_rect.list_partition_pointer->get(id_segment), branch_pointer[id_branch_b].m_rect.list_partition_pointer->get(id_segment));

				if (distance_power_segment_current > rectangle_combined.difference_power_each_segment_max_vector[id_segment]) {

					rectangle_combined.difference_power_each_segment_max_vector[id_segment] = distance_power_segment_current;
					rectangle_combined.difference_each_segment_max_vector[id_segment] = sqrtl(rectangle_combined.difference_power_each_segment_max_vector[id_segment]);

					rectangle_combined.id_branch_segment_min_vector[id_segment] = id_branch_a;
					rectangle_combined.id_branch_segment_max_vector[id_segment] = id_branch_b;
		...
		if (rectangle_combined.list_partition_local_min_pointer->size() == branch_pointer[0].m_rect.list_partition_pointer->size()) {
			/*...........................................................................................................................................................*/
#ifdef _DEBUG
			APLA::assert_has_same_endpoints(*branch_pointer[0].m_rect.list_partition_pointer, *rectangle_combined.list_partition_local_min_pointer);
#endif
			/*...........................................................................................................................................................*/

			rectangle_combined.list_partition_local_min_pointer->get(id_segment) = branch_pointer[rectangle_combined.id_branch_segment_min_vector[id_segment]].m_rect.list_partition_pointer->get(id_segment);
			rectangle_combined.list_partition_local_max_pointer->get(id_segment) = branch_pointer[rectangle_combined.id_branch_segment_max_vector[id_segment]].m_rect.list_partition_pointer->get(id_segment);
		}
		else {
			rectangle_combined.list_partition_local_min_pointer->emplace_back(branch_pointer[rectangle_combined.id_branch_segment_min_vector[id_segment]].m_rect.list_partition_pointer->get(id_segment));
			rectangle_combined.list_partition_local_max_pointer->emplace_back(branch_pointer[rectangle_combined.id_branch_segment_max_vector[id_segment]].m_rect.list_partition_pointer->get(id_segment));
		}
		/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
	}
	/*==========================================================================================================================================================*/
	...
	//node.rectangle_in_node.volume_minmax_segment_sqrt = accumulate(node.rectangle_in_node.difference_each_segment_max_vector.begin(), node.rectangle_in_node.difference_each_segment_max_vector.end(), 0);
	rectangle_combined.volume_minmax_segment = accumulate(rectangle_combined.difference_power_each_segment_max_vector.begin(), rectangle_combined.difference_power_each_segment_max_vector.end(), 0);
	rectangle_combined.volume_minmax_segment_sqrt = sqrtl(rectangle_combined.volume_minmax_segment);

	return rectangle_combined.volume_minmax_segment_sqrt;
}
...
long double& RTREE_PARTITION_QUAL::get_convex_hull_by_distance_approximation_local(T& const rectangle_in_node) {
	...
	/*...........................................................................................................................................................*/
#ifdef _DEBUG
	if (rectangle_in_node.list_partition_local_min_pointer->size() == rectangle_in_node.list_partition_pointer_vector[0]->size()) {
		APLA::assert_has_same_endpoints(*rectangle_in_node.list_partition_local_min_pointer, *rectangle_in_node.list_partition_pointer_vector[0]);
	}
	else {
		assert(rectangle_in_node.list_partition_local_min_pointer->empty() && !rectangle_in_node.list_partition_pointer_vector[0]->empty());
	}
	assert_rectangle(rectangle_in_node);
#endif
	/*...........................................................................................................................................................*/
	long double distance_power_segment_current = -INF;
	rectangle_in_node.difference_each_segment_max_vector.assign(rectangle_in_node.list_partition_pointer_vector[0]->size(), -INF);
	rectangle_in_node.difference_power_each_segment_max_vector.assign(rectangle_in_node.list_partition_pointer_vector[0]->size(), -INF);
	rectangle_in_node.id_branch_segment_min_vector.assign(rectangle_in_node.list_partition_pointer_vector[0]->size(), -1);
	rectangle_in_node.id_branch_segment_max_vector.assign(rectangle_in_node.list_partition_pointer_vector[0]->size(), -1);

	for (int id_segment = 0; id_segment < rectangle_in_node.list_partition_pointer_vector[0]->size(); id_segment++) {

		for (int id_branch_a = 0; id_branch_a < rectangle_in_node.list_partition_pointer_vector.size() - 1; ++id_branch_a) {
			for (int id_branch_b = id_branch_a + 1; id_branch_b < rectangle_in_node.list_partition_pointer_vector.size(); ++id_branch_b) {

				/*...........................................................................................................................................................*/
#ifdef _DEBUG
				assert(rectangle_in_node.difference_power_each_segment_max_vector.size() == rectangle_in_node.difference_each_segment_max_vector.size());
				APLA::assert_has_same_endpoints(*rectangle_in_node.list_partition_local_min_pointer, *rectangle_in_node.list_partition_local_max_pointer);
#endif
				/*...........................................................................................................................................................*/

				distance_power_segment_current = APLA::get_segment_distance_LB(rectangle_in_node.list_partition_pointer_vector[id_branch_a]->get(id_segment), rectangle_in_node.list_partition_pointer_vector[id_branch_b]->get(id_segment));

				if (distance_power_segment_current > rectangle_in_node.difference_power_each_segment_max_vector[id_segment]) {

					rectangle_in_node.difference_power_each_segment_max_vector[id_segment] = distance_power_segment_current;
					rectangle_in_node.difference_each_segment_max_vector[id_segment] = sqrtl(rectangle_in_node.difference_power_each_segment_max_vector[id_segment]);

					rectangle_in_node.id_branch_segment_min_vector[id_segment] = id_branch_a;
					rectangle_in_node.id_branch_segment_max_vector[id_segment] = id_branch_b;
		...
		if (rectangle_in_node.list_partition_local_min_pointer->size() == rectangle_in_node.list_partition_pointer_vector[0]->size()) {
			/*...........................................................................................................................................................*/
#ifdef _DEBUG
			APLA::assert_has_same_endpoints(*rectangle_in_node.list_partition_pointer_vector[0], *rectangle_in_node.list_partition_local_min_pointer);
#endif
			/*...........................................................................................................................................................*/

			rectangle_in_node.list_partition_local_min_pointer->get(id_segment) = rectangle_in_node.list_partition_pointer_vector[rectangle_in_node.id_branch_segment_min_vector[id_segment]]->get(id_segment);
			rectangle_in_node.list_partition_local_max_pointer->get(id_segment) = rectangle_in_node.list_partition_pointer_vector[rectangle_in_node.id_branch_segment_max_vector[id_segment]]->get(id_segment);
		}
		else {
			rectangle_in_node.list_partition_local_min_pointer->emplace_back(rectangle_in_node.list_partition_pointer_vector[rectangle_in_node.id_branch_segment_min_vector[id_segment]]->get(id_segment));
			rectangle_in_node.list_partition_local_max_pointer->emplace_back(rectangle_in_node.list_partition_pointer_vector[rectangle_in_node.id_branch_segment_max_vector[id_segment]]->get(id_segment));
		}
		/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
	}
	/*==========================================================================================================================================================*/
	...
	//node.rectangle_in_node.volume_minmax_segment_sqrt = accumulate(node.rectangle_in_node.difference_each_segment_max_vector.begin(), node.rectangle_in_node.difference_each_segment_max_vector.end(), 0);
	rectangle_in_node.volume_minmax_segment = accumulate(rectangle_in_node.difference_power_each_segment_max_vector.begin(), rectangle_in_node.difference_power_each_segment_max_vector.end(), 0);
	rectangle_in_node.volume_minmax_segment_sqrt = sqrtl(rectangle_in_node.volume_minmax_segment);

	return rectangle_in_node.volume_minmax_segment_sqrt;
}
...
long double RTREE_PARTITION_QUAL::compute_distance_max_between_rectangle_node(T& const rectangle_in_node_0, Y& const rectangle_in_node_1) {
	...
	for (int id_branch_a = 0; id_branch_a < rectangle_in_node_0.list_original_pointer_vector.size(); ++id_branch_a) {
		for (int id_branch_b = 0; id_branch_b < rectangle_in_node_1.list_original_pointer_vector.size(); ++id_branch_b) {
			/*...........................................................................................................................................................*/
#ifdef _DEBUG
			APLA::assert_has_same_endpoints(*rectangle_in_node_0.list_partition_pointer_vector[id_branch_a], *rectangle_in_node_1.list_partition_pointer_vector[id_branch_b]);
#endif
			/*...........................................................................................................................................................*/

			distance_current = APLA::get_sqrt_distance_sapla_same_endpoints(*rectangle_in_node_0.list_partition_pointer_vector[id_branch_a], *rectangle_in_node_1.list_partition_pointer_vector[id_branch_b]);
...
long double RTREE_PARTITION_QUAL::get_convex_hull_by_entry(const T* const branch_pointer, const Y& const size_branch, U& const rectangle_combined) {
...
#ifdef _DEBUG
	//ASSERT(a_rect);
	assert(branch_pointer[0].m_rect.list_original_pointer);
	assert(size_branch > 0);
	assert(branch_pointer);
	assert_branch_rect_in_node(rectangle_combined, branch_pointer);
	//assert(rectangle_new.original_time_series_vector_pointer.size() == rectangle_new.reconstruct_time_series_vector_pointer.size() && rectangle_new.list_partition_pointer->size() >= rectangle_new.list_original_pointer->size());
	...
	case 0: {// global distance
		...
		/*...........................................................................................................................................................*/
		rectangle_combined.volume_minmax_whole_sqrt = rectangle_combined.volume_minmax_whole = -INF;
		if (rectangle_combined.list_partition_global_min_pointer->size() != branch_pointer[0].m_rect.list_partition_pointer->size()) {
			rectangle_combined.list_partition_global_min_pointer->clear();
			rectangle_combined.list_partition_global_max_pointer->clear();
		}
	...
	default: {
		...
		APLA::assert_has_same_endpoints(*rectangle_combined.list_partition_local_min_pointer, *rectangle_combined.list_partition_local_max_pointer);
		APLA::assert_has_same_endpoints(*rectangle_combined.list_partition_global_min_pointer, *rectangle_combined.list_partition_global_max_pointer);
		APLA::assert_has_same_endpoints(*rectangle_combined.list_partition_local_min_pointer, *rectangle_combined.list_partition_global_min_pointer);
#endif
		/*...........................................................................................................................................................*/
		...
		if (rectangle_combined.list_partition_global_min_pointer->size() != branch_pointer[0].m_rect.list_partition_pointer->size()) {
			rectangle_combined.list_partition_local_min_pointer->clear();
			rectangle_combined.list_partition_local_max_pointer->clear();
			rectangle_combined.list_partition_global_min_pointer->clear();
			rectangle_combined.list_partition_global_max_pointer->clear();
		}
...
long double RTREE_PARTITION_QUAL::get_convex_hull_by_node(T& const node) {
	...
	if (node.IsLeaf()) {// Leaf Node from Entry
		/*............................................................*/
#ifdef _DEBUG
		assert(node.rectangle_in_node.size_entry() == node.m_count && node.m_count > 1);
#endif
		/*............................................................*/
		...
		/*............................................................*/
#ifdef _DEBUG
		assert(node.m_count <= node.rectangle_in_node.size_entry());
#endif
		/*............................................................*/
...
long double RTREE_PARTITION_QUAL::get_convex_hull_by_node_rectangle(T& const node) {
	...
	//switch (option_distance) {
	//case 0: {// global distance
	//	node.rectangle_in_node.volume_minmax_whole_sqrt = node.rectangle_in_node.volume_minmax_whole = -INF;
	//	if (node.rectangle_in_node.list_partition_global_min_pointer->size() != node.rectangle_in_node.list_partition_pointer_vector[0]->size()) {
	//		node.rectangle_in_node.list_partition_global_min_pointer->clear();
	//		node.rectangle_in_node.list_partition_global_max_pointer->clear();
	//	}
	...
	return node.rectangle_in_node.volume_minmax_whole_sqrt;
...
long double RTREE_PARTITION_QUAL::get_convex_hull_by_rectangle(T& const rectangle) {
	...
	case 0: {// global distance
		...
		if (rectangle.size_entry() == 0) {
			return 0;
		}
		else if (rectangle.size_entry() == 1) {
			return rectangle.volume_minmax_whole_sqrt = rectangle.volume_minmax_whole = 0;
		}

		rectangle.volume_minmax_whole_sqrt = rectangle.volume_minmax_whole = -INF;

		/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/

		if (rectangle.list_partition_global_min_pointer->size() != rectangle.list_partition_pointer_vector[0]->size()) {
			rectangle.list_partition_global_min_pointer->clear();
			rectangle.list_partition_global_max_pointer->clear();
		}
	...
	default: {
		...
		if (rectangle.size_entry() == 0) {
			...
			return rectangle.volume_minmax_whole_sqrt = rectangle.volume_minmax_whole = rectangle.volume_minmax_segment_sqrt = rectangle.volume_minmax_segment = rectangle.volume_minmax_point_sqrt = rectangle.volume_minmax_point = 0;
		}
		...
		if (rectangle.list_partition_global_min_pointer->size() != rectangle.list_partition_pointer_vector[0]->size()) {
			rectangle.list_partition_local_min_pointer->clear();
			rectangle.list_partition_local_max_pointer->clear();
			rectangle.list_partition_global_min_pointer->clear();
			rectangle.list_partition_global_max_pointer->clear();
		}
	...
	return rectangle.volume_minmax_whole_sqrt;
}
...
//            3 This function only used in Split Node() function
...
void RTREE_PARTITION_QUAL::choose_partition_by_distance(const T& const a_minFill, Y& const a_parVars) {
	...
	//assert_same_SAPLA(*a_parVars.m_coverSplit.list_partition_global_min_pointer, *a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_0]);
	//assert_same_SAPLA(*a_parVars.m_coverSplit.list_partition_global_max_pointer, *a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_1]);
#endif
	/*...................................................................................................................................*/

	
	if(!a_parVars.m_branchBuf[0].m_child){//branch all entry
		...
		assert(a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_0] == a_parVars.m_branchBuf[a_parVars.m_coverSplit.seed_0].m_rect.list_partition_pointer);
		assert(a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_1] == a_parVars.m_branchBuf[a_parVars.m_coverSplit.seed_1].m_rect.list_partition_pointer);
		long double max_distance = APLA::get_sqrt_distance_sapla_same_endpoints(*a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_0], *a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_1]);
#endif
		/*...................................................................................................................................*/
		for (int id_branch = 0; id_branch < a_parVars.m_total; ++id_branch) {
			if (PartitionVars::NOT_TAKEN == a_parVars.m_partition[id_branch]) {
				long double distance_0 = APLA::get_sqrt_distance_sapla_same_endpoints(*a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_0], *a_parVars.m_branchBuf[id_branch].m_rect.list_partition_pointer);
				long double distance_1 = APLA::get_sqrt_distance_sapla_same_endpoints(*a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_1], *a_parVars.m_branchBuf[id_branch].m_rect.list_partition_pointer);
				/*..................................................................................................................................*/
#ifdef _DEBUG
				assert(max_distance >= distance_0 && max_distance >= distance_1);
				assert(a_parVars.m_coverSplit.list_partition_pointer_vector[id_branch] == a_parVars.m_branchBuf[id_branch].m_rect.list_partition_pointer);
#endif
				/*...................................................................................................................................*/
...
//#endif
//					/*...................................................................................................................................*/
//					distance_0 = APLA::get_sqrt_distance_sapla_same_endpoints(*a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_0], *a_parVars.m_branchBuf[id_branch].m_rect.list_partition_pointer);
//					distance_1 = APLA::get_sqrt_distance_sapla_same_endpoints(*a_parVars.m_coverSplit.list_partition_pointer_vector[a_parVars.m_coverSplit.seed_1], *a_parVars.m_branchBuf[id_branch].m_rect.list_partition_pointer);
//				}
//				else {
//					assert(0);
//				}
...
inline void RTREE_PARTITION_QUAL::InitParVars(PartitionVars* a_parVars, int a_maxRects, int a_minFill)
{
	...
	a_parVars->m_area[0] = a_parVars->m_area[1] = (ELEMTYPEREAL)0; //  the are of each two group
...
long double RTREE_PARTITION_QUAL::pick_seeds(T& const a_parVars) {
	...
	if (!a_parVars.m_branchBuf[0].m_child) {// all branch are entry. !branch.m_child && branch.m_data != INF
		/*...........................................................................................................................................................*/
#ifdef _DEBUG
		assert(is_branch_pointer_all_entry(a_parVars.m_total, a_parVars.m_branchBuf));
#endif
	...
	/*====================================   For global approximation. Bubble sort: compare every two of branches  =================================================*/
		...
		for (int id_branch_a = 0; id_branch_a < a_parVars.m_total - 1; ++id_branch_a) {
			...
			/*...........................................................................................................................................................*/
				...
				//	assert(0);
				//	distance_current = APLA::get_sqrt_distance_sapla_same_endpoints(*a_parVars.m_branchBuf[id_branch_a].m_rect.list_partition_pointer, *a_parVars.m_branchBuf[id_branch_b].m_rect.list_partition_pointer);
				//}
				//else {
				//	assert(0);
				//}
	...
	/*------------  get two approximation with max SAPLA distance  -------------*/
	//rectangle_combined.list_partition_global_min_pointer->copy(*branch_pointer[a_parVars.seed_0].m_rect.list_partition_pointer);
	//rectangle_combined.list_partition_global_max_pointer->copy(*branch_pointer[a_parVars.seed_1].m_rect.list_partition_pointer);
	/*---------------------------------------------------------------------------*/
...
inline bool RTREE_PARTITION_QUAL::is_rectangle_entry(const T& const rectangle) {
	if (rectangle.list_original_pointer && rectangle.original_time_series_pointer_vector.empty()) return true;
	else {
		assert(0);
		return false;
	}
...
inline bool RTREE_PARTITION_QUAL::is_rectangle_node(const T& const rectangle) {
	if (!rectangle.list_original_pointer && rectangle.list_partition_global_min_pointer) return true;
	else return false;
}
...
inline bool RTREE_PARTITION_QUAL::is_branch_entry(const T& const branch) {
	if (!branch.m_child && branch.m_data != INF) {
		assert(branch.m_rect.list_partition_pointer && branch.m_rect.original_time_series_vector_pointer);
		return true;
	}
	assert(0);
	return false;
...
bool RTREE_PARTITION_QUAL::assert_rectangle(const T& const rectangle) {
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	if (rectangle.list_original_pointer != nullptr) {//entry
		//assert(rectangle.original_time_series_vector_pointer->size() == rectangle.reconstruct_time_series_vector_pointer->size());
		assert(rectangle.list_original_pointer->size() <= rectangle.list_partition_pointer->size());
		assert(rectangle.original_time_series_pointer_vector.empty()  && rectangle.list_original_pointer_vector.empty() && rectangle.list_partition_pointer_vector.empty());
	}
		...
		assert(!rectangle.list_original_pointer && !rectangle.list_original_pointer);
		assert(rectangle.list_partition_global_min_pointer && rectangle.list_partition_global_max_pointer);
		//assert(!rectangle.original_time_series_pointer_vector.empty() && !rectangle.reconstruct_time_series_pointer_vector.empty() && !rectangle.list_original_pointer_vector.empty() && !rectangle.list_partition_pointer_vector.empty());
	}
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	//assert(rectangle.reconstruct_time_series_pointer_vector.size() == rectangle.original_time_series_pointer_vector.size());
	assert(rectangle.list_original_pointer_vector.size() == rectangle.original_time_series_pointer_vector.size());
	assert(rectangle.list_original_pointer_vector.size() == rectangle.list_partition_pointer_vector.size());
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	//assert(rectangle.value_point_max_vector.size() == rectangle.value_point_min_vector.size());
	assert(rectangle.id_branch_segment_min_vector.size() == rectangle.id_branch_segment_max_vector.size());
	assert(rectangle.difference_each_segment_max_vector.size() == rectangle.difference_power_each_segment_max_vector.size());

	if (rectangle.list_partition_local_min_pointer) {
		APLA::assert_has_same_endpoints(*rectangle.list_partition_local_min_pointer, *rectangle.list_partition_local_max_pointer);
		APLA::assert_has_same_endpoints(*rectangle.list_partition_global_min_pointer, *rectangle.list_partition_global_max_pointer);

		if (rectangle.list_partition_local_min_pointer->size() > 0 && rectangle.list_partition_global_min_pointer->size() > 0) {
			APLA::assert_has_same_endpoints(*rectangle.list_partition_local_min_pointer, *rectangle.list_partition_global_min_pointer);
		}
	}
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	/*for (int id_branch = 0; id_branch < rectangle.list_partition_pointer_vector.size(); id_branch++) {
		assert_same_vector(*rectangle.original_time_series_pointer_vector[id_branch], rectangle.original_time_series_vector_vector[id_branch]);
	}*/

	if (!rectangle.list_partition_pointer_vector.empty()) {
		for (int id_branch_a = 0; id_branch_a < rectangle.list_partition_pointer_vector.size() - 1; ++id_branch_a) {
			for (int id_branch_b = id_branch_a + 1; id_branch_b < rectangle.list_partition_pointer_vector.size(); ++id_branch_b) {
				APLA::assert_has_same_endpoints(*rectangle.list_partition_pointer_vector[id_branch_a], *rectangle.list_partition_pointer_vector[id_branch_b]);
				//APLA::assert_has_same_endpoints(*rectangle.list_original_pointer_vector[id_branch_a], *rectangle.list_original_pointer_vector[id_branch_b]);
				if(representation_option != 3)
				assert(rectangle.list_original_pointer_vector[id_branch_a]->size() == rectangle.list_original_pointer_vector[id_branch_b]->size());
			}
		}
	}
...
bool RTREE_PARTITION_QUAL::assert_rectangle_same(const T& const rectangle_0, const Y& const rectangle_1) {
	...
	if (rectangle_0.original_time_series_vector_pointer) {// Entry rectangle
		assert(rectangle_0.original_time_series_vector_pointer == rectangle_1.original_time_series_vector_pointer);
		//assert(rectangle_0.reconstruct_time_series_vector_pointer == rectangle_1.reconstruct_time_series_vector_pointer);
		assert(rectangle_0.list_original_pointer == rectangle_1.list_original_pointer);
		assert(rectangle_0.list_partition_pointer == rectangle_1.list_partition_pointer);
	}
	else {// Internal rectangle


		if (rectangle_0.list_partition_global_min_pointer) {// has convex hull volume
			assert(rectangle_0.volume_minmax_segment == rectangle_1.volume_minmax_segment && rectangle_0.volume_minmax_segment_sqrt == rectangle_1.volume_minmax_segment_sqrt);
			assert(rectangle_0.volume_minmax_whole == rectangle_1.volume_minmax_whole && rectangle_0.volume_minmax_whole_sqrt == rectangle_1.volume_minmax_whole_sqrt);
			assert(rectangle_0.volume_minmax_point == rectangle_1.volume_minmax_point && rectangle_0.volume_minmax_point_sqrt == rectangle_1.volume_minmax_point_sqrt);
			assert(rectangle_0.seed_0 == rectangle_1.seed_0 && rectangle_0.seed_1 == rectangle_1.seed_1);

			assert_same_vector(rectangle_0.value_point_min_vector, rectangle_1.value_point_min_vector);
			//(rectangle_0.value_point_max_vector, rectangle_1.value_point_max_vector);
			assert_same_vector(rectangle_0.id_branch_segment_min_vector, rectangle_1.id_branch_segment_min_vector);
			assert_same_vector(rectangle_0.id_branch_segment_max_vector, rectangle_1.id_branch_segment_max_vector);
			assert_same_vector(rectangle_0.difference_each_segment_max_vector, rectangle_1.difference_each_segment_max_vector);
			assert_same_vector(rectangle_0.difference_power_each_segment_max_vector, rectangle_1.difference_power_each_segment_max_vector);

			assert_same_SAPLA(*rectangle_0.list_partition_global_min_pointer, *rectangle_1.list_partition_global_min_pointer);
			assert_same_SAPLA(*rectangle_0.list_partition_global_max_pointer, *rectangle_1.list_partition_global_max_pointer);
			if (rectangle_0.list_partition_local_min_pointer) {
				assert_same_SAPLA(*rectangle_0.list_partition_local_min_pointer, *rectangle_1.list_partition_local_min_pointer);
				assert_same_SAPLA(*rectangle_0.list_partition_local_max_pointer, *rectangle_1.list_partition_local_max_pointer);
			}
		}

		// vector of pointer
		assert(rectangle_0.original_time_series_pointer_vector.size() == rectangle_1.original_time_series_pointer_vector.size());
		//assert(rectangle_0.reconstruct_time_series_pointer_vector.size() == rectangle_1.reconstruct_time_series_pointer_vector.size());
		assert(rectangle_0.list_original_pointer_vector.size() == rectangle_1.list_original_pointer_vector.size());
		assert(rectangle_0.list_partition_pointer_vector.size() == rectangle_1.list_partition_pointer_vector.size());
		for (int id_branch = 0; id_branch < rectangle_0.list_original_pointer_vector.size(); id_branch++) {
			assert(rectangle_0.original_time_series_pointer_vector[id_branch] == rectangle_1.original_time_series_pointer_vector[id_branch]);
			...
			assert(rectangle_0.list_original_pointer_vector[id_branch] == rectangle_1.list_original_pointer_vector[id_branch]);
			assert(rectangle_0.list_partition_pointer_vector[id_branch] == rectangle_1.list_partition_pointer_vector[id_branch]);
		}
	}
	...
	if (size_array > 0) {
		for (int id_branch_a = 0; id_branch_a < size_array - 1; ++id_branch_a) {
			for (int id_branch_b = id_branch_a + 1; id_branch_b < size_array; ++id_branch_b) {
				...
				if (branch_pointer[id_branch_a].m_rect.original_time_series_vector_pointer && branch_pointer[id_branch_b].m_rect.original_time_series_vector_pointer) {
					assert(branch_pointer[id_branch_a].m_rect.original_time_series_vector_pointer->size() == branch_pointer[id_branch_b].m_rect.original_time_series_vector_pointer->size());
					...
					if(representation_option != 3)
					assert(branch_pointer[id_branch_a].m_rect.list_original_pointer->size() == branch_pointer[id_branch_b].m_rect.list_original_pointer->size());
					APLA::assert_has_same_endpoints(*branch_pointer[id_branch_a].m_rect.list_partition_pointer, *branch_pointer[id_branch_b].m_rect.list_partition_pointer);
				}
				//APLA::assert_has_same_endpoints(*a_node->m_branch[id_branch_a].m_rect.list_original_pointer, *a_node->m_branch[id_branch_b].m_rect.list_original_pointer);
				//APLA::assert_has_same_endpoints(*a_node->rectangle_in_node.list_original_pointer_vector[id_branch_a], *a_node->rectangle_in_node.list_original_pointer_vector[id_branch_b]);
			}
		}
	}
	return true;
...
bool RTREE_PARTITION_QUAL::assert_branch_rect_in_node(const T& const rectangle, Y* branch_pointer) {
	...
	if (!branch_pointer || branch_pointer->m_child) {
		assert(branch_pointer->m_data == INF);
		return true;
	}

	assert_branch_array(rectangle.list_original_pointer_vector.size(), branch_pointer);

	for (int id_branch = 0; id_branch < rectangle.list_original_pointer_vector.size(); id_branch++) {
		...
		//assert_same_vector(*rectangle.original_time_series_pointer_vector[id_branch], *branch_pointer[id_branch].m_rect.original_time_series_vector_pointer);
		//assert(a_node->rectangle_in_node.original_time_series_pointer_vector[id_branch] == &(branch_pointer[id_branch].m_rect.original_time_series_vector_pointer));

		// pointer same address
		assert(rectangle.original_time_series_pointer_vector[id_branch] == branch_pointer[id_branch].m_rect.original_time_series_vector_pointer);
		...
		assert(rectangle.list_partition_pointer_vector[id_branch] == branch_pointer[id_branch].m_rect.list_partition_pointer);
		assert(rectangle.list_original_pointer_vector[id_branch] == branch_pointer[id_branch].m_rect.list_original_pointer);

		assert_same_SAPLA(*rectangle.list_partition_pointer_vector[id_branch], *branch_pointer[id_branch].m_rect.list_partition_pointer);
		assert_same_SAPLA(*rectangle.list_original_pointer_vector[id_branch], *branch_pointer[id_branch].m_rect.list_original_pointer);
		/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	}
...
bool RTREE_PARTITION_QUAL::assert_node_leaf(T& const a_node) {
	...
	assert(a_node.IsLeaf() && a_node.m_branch && a_node.rectangle_in_node.original_time_series_pointer_vector.size() == a_node.m_count);
	//assert(a_node.rectangle_in_node.reconstruct_time_series_pointer_vector.size() == a_node.m_count);

	//assert(a_node.rectangle_in_node.original_time_series_vector_vector.size() == a_node.rectangle_in_node.original_time_series_pointer_vector.size());
	//assert(a_node.rectangle_in_node.list_original_pointer_vector.size() == a_node.rectangle_in_node.original_time_series_pointer_vector.size());
	//assert(a_node.rectangle_in_node.list_original_pointer_vector.size() == a_node.rectangle_in_node.list_partition_pointer_vector.size());

	/*assert(a_node.rectangle_in_node.value_point_max_vector.size() == a_node.rectangle_in_node.value_point_min_vector.size());
	assert(a_node.rectangle_in_node.id_branch_segment_min_vector.size() == a_node.rectangle_in_node.id_branch_segment_max_vector.size());
	assert(a_node.rectangle_in_node.difference_each_segment_max_vector.size() == a_node.rectangle_in_node.difference_power_each_segment_max_vector.size());*/

	/*APLA::assert_has_same_endpoints(*a_node.rectangle_in_node.list_partition_local_min_pointer, *a_node.rectangle_in_node.list_partition_local_max_pointer);
	APLA::assert_has_same_endpoints(*a_node.rectangle_in_node.list_partition_global_min_pointer, *a_node.rectangle_in_node.list_partition_global_max_pointer);
	APLA::assert_has_same_endpoints(*a_node.rectangle_in_node.list_partition_local_min_pointer, *a_node.rectangle_in_node.list_partition_global_min_pointer);*/

	assert_node_branch_pointer(a_node, a_node.m_branch);
	assert_branch_rect_in_node(a_node.rectangle_in_node, a_node.m_branch);

	//for (int id_branch = 0; id_branch < a_node->m_count; id_branch++) {
	//	APLA::assert_has_same_endpoints(*a_node->m_branch[0].m_rect.list_partition_pointer, *a_node->m_branch[id_branch].m_rect.list_partition_pointer);
	//	//APLA::assert_has_same_endpoints(*a_node->m_branch[0].m_rect.list_original_pointer, *a_node->m_branch[id_branch].m_rect.list_original_pointer);
	...
	//	assert_same_vector(*a_node->rectangle_in_node.original_time_series_pointer_vector[id_branch], a_node->m_branch[id_branch].m_rect.original_time_series_vector_pointer);
	//	assert_same_vector(*a_node->rectangle_in_node.original_time_series_pointer_vector[id_branch], a_node->rectangle_in_node.original_time_series_vector_vector[id_branch]);
	//	//assert(a_node->rectangle_in_node.original_time_series_pointer_vector[id_branch] == &(a_node->m_branch[id_branch].m_rect.original_time_series_vector_pointer));

	//	// pointer same address
	//	assert(a_node->rectangle_in_node.list_partition_pointer_vector[id_branch] == a_node->m_branch[id_branch].m_rect.list_partition_pointer);
	//	assert(a_node->rectangle_in_node.list_original_pointer_vector[id_branch] == a_node->m_branch[id_branch].m_rect.list_original_pointer);

	//	assert_same_SAPLA(*a_node->rectangle_in_node.list_partition_pointer_vector[id_branch], *a_node->m_branch[id_branch].m_rect.list_partition_pointer);
	//	assert_same_SAPLA(*a_node->rectangle_in_node.list_original_pointer_vector[id_branch], *a_node->m_branch[id_branch].m_rect.list_original_pointer);
	//	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	//
	//	assert_rectangle(a_node->m_branch[id_branch].m_rect);
	//}

	//for (int id_branch_a = 0; id_branch_a < a_node->rectangle_in_node.list_partition_pointer_vector.size() - 1; ++id_branch_a) {
	//	for (int id_branch_b = id_branch_a + 1; id_branch_b < a_node->rectangle_in_node.list_partition_pointer_vector.size(); ++id_branch_b) {
	...
	//		APLA::assert_has_same_endpoints(*a_node->m_branch[id_branch_a].m_rect.list_partition_pointer, *a_node->m_branch[id_branch_b].m_rect.list_partition_pointer);
	//		assert(a_node->m_branch[id_branch_a].m_rect.list_original_pointer->size() == a_node->m_branch[id_branch_b].m_rect.list_original_pointer->size());
	//		assert(a_node->rectangle_in_node.list_original_pointer_vector[id_branch_a]->size() == a_node->rectangle_in_node.list_original_pointer_vector[id_branch_b]->size());
	//		//APLA::assert_has_same_endpoints(*a_node->m_branch[id_branch_a].m_rect.list_original_pointer, *a_node->m_branch[id_branch_b].m_rect.list_original_pointer);
	//		APLA::assert_has_same_endpoints(*a_node->rectangle_in_node.list_partition_pointer_vector[id_branch_a], *a_node->rectangle_in_node.list_partition_pointer_vector[id_branch_b]);
	//		//APLA::assert_has_same_endpoints(*a_node->rectangle_in_node.list_original_pointer_vector[id_branch_a], *a_node->rectangle_in_node.list_original_pointer_vector[id_branch_b]);
	//	}
	//}
	...
	case 8: {// Initial 200706
		if (&a_node) {
			...
			if (a_node.IsInternalNode()) {  // Internal node
				...
				for (int id_branch = 0; id_branch < a_node.m_count; ++id_branch) {
					size_entry_sub += a_node.m_branch[id_branch].m_rect.size_entry();
					assert(a_node.m_branch[id_branch].branch_to_parent_node_pointer == &a_node);
				...
				assert(a_node.rectangle_in_node.size_entry() == size_entry_sub);
			}
			else { // Leaf node
				assert(a_node.rectangle_in_node.size_entry() == a_node.m_count);
				assert_node_leaf(a_node);
			}

			if (a_node.rectangle_in_node.size_entry() > 0) {
				for (int id_branch_a = 0; id_branch_a < a_node.rectangle_in_node.size_entry() - 1; ++id_branch_a) {
					for (int id_branch_b = id_branch_a + 1; id_branch_b < a_node.rectangle_in_node.size_entry(); ++id_branch_b) {
